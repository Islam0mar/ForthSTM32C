...
# 107 "/usr/lib/llvm-6.0/lib/clang/6.0.0/include/stdint.h" 3
typedef long int int64_t;

typedef long unsigned int uint64_t;
# 121 "/usr/lib/llvm-6.0/lib/clang/6.0.0/include/stdint.h" 3
typedef int64_t int_least64_t;
typedef uint64_t uint_least64_t;
typedef int64_t int_fast64_t;
typedef uint64_t uint_fast64_t;
# 178 "/usr/lib/llvm-6.0/lib/clang/6.0.0/include/stdint.h" 3
typedef int int32_t;
...
# 195 "/usr/lib/llvm-6.0/lib/clang/6.0.0/include/stdint.h" 3
typedef int32_t int_least32_t;
typedef uint32_t uint_least32_t;
typedef int32_t int_fast32_t;
typedef uint32_t uint_fast32_t;
# 216 "/usr/lib/llvm-6.0/lib/clang/6.0.0/include/stdint.h" 3
...
# 260 "/usr/lib/llvm-6.0/lib/clang/6.0.0/include/stdint.h" 3
...
_Bool addToTail(DictionaryEntry item, Dictionary *pd);
DictionaryNode *findDictionaryItem(Dictionary *pd, char *name);
_Bool deleteDictionaryItem(Dictionary *pd, char *name);
_Bool dictionaryEmpty(Dictionary *pd);
void createDictionary(Dictionary *pd);
int dictionarySize(Dictionary *pd);
void clearDictionary(Dictionary *pd);
_Bool addDictionaryEntry(char *name, uint8_t flags, uint8_t length,
                        FuncPtr ptr[], Dictionary *pd);
# 5 "./macros.h" 2
# 11 "<stdin>" 2
# 41 "<stdin>"
void quit() { longjmp(env, 0); } const DictionaryNode quit_node = {.next = ((void*)0), .entry = {.flags = 0|0x10U, .length = 1, .ptr = (void*)&quit}};



void key() { PushTOS(); tos = TIBReadChar();} const DictionaryNode key_node = {.next = ((void*)0), .entry = {.flags = 0|0x10U, .length = 1, .ptr = (void*)&key}};





void is_key() { PushTOS(); tos = (TIBEmpty()) ? 0 : 1;} const DictionaryNode is_key_node = {.next = ((void*)0), .entry = {.flags = 0|0x10U, .length = 1, .ptr = (void*)&is_key}};




void word() { static uint8_t i; for (i = 0;;) { pad[i] = TIBReadChar(); switch (pad[i]) { case ' ': { break; } case '\\': { do { pad[i] = TIBReadChar(); } while (pad[i] != '\n'); break; } case '(': { do { pad[i] = TIBReadChar(); } while (pad[i] != ')'); break; } default: { do { ++i; if(i>=PAD_SIZE){ ForthError("PAD size over flow"); } pad[i] = TIBReadChar(); } while (pad[i] != ' '); pad[i] = '\0'; break; } } } PushTOS(); PushPSP(pad); tos = i; } const DictionaryNode word_node = {.next = ((void*)0), .entry = {.flags = 0|0x10U, .length = 1, .ptr = (void*)&word}};
# 97 "<stdin>"
void paren_find() { PopTOS(); tos = findDictionaryItem(GetDictPtr(), tos); } const DictionaryNode paren_find_node = {.next = ((void*)0), .entry = {.flags = 0|0x10U, .length = 1, .ptr = (void*)&paren_find}};





void tcfa() { tos = tos->ptr; } const DictionaryNode tcfa_node = {.next = ((void*)0), .entry = {.flags = 0|0x10U, .length = 1, .ptr = (void*)&tcfa}};




void tdfa() { os = tos->ptr; } const DictionaryNode tdfa_node = {.next = ((void*)0), .entry = {.flags = 0|0x10U, .length = 1, .ptr = (void*)&tdfa}};
# 134 "<stdin>"
void comma() { if(((state&0x02U)!=0)){ } else{ HA data_ptr = relloc(data_ptr, 2*sizeof(FuncPtr)); } } const DictionaryNode comma_node = {.next = ((void*)0), .entry = {.flags = 0|0x10U, .length = 1, .ptr = (void*)&comma}}
# 159 "<stdin>"
void interpret() { word(); paren_find(); if(tos == ((void*)0)) { if(EXECUTE_STATE) { tos = StrToInt(); } else{ tos = ; comma(); tos = StrToInt(); comma(); } } if((tos->entry.flags & 0x80U) != 0){ if((tos->entry.flags & 0x40U) != 0 && !STATE){ char err[PAD_SIZE+27] = "COMPILATION ONLY WORD, <"; strcat(err, pad); strcat(err, ">."); ForthError(err); }else if((tos->entry.flags & F_CODE) != 0){ (*tos->ptr)(); } else { static uint8_t i; for(i = 0;i< tos->entry.length;i++){ (tos->entry.ptr[i])(); } } } else if ((tos->entry.flags & 0x40U) != 0 && !STATE){ char err[PAD_SIZE+26] = "COMPILATION ONLY WORD <"; strcat(err, pad); strcat(err, ">."); ForthError(err); } else if ((tos->entry.flags & 0x40U) != 0 || !STATE){ if((tos->entry.flags & F_CODE) != 0){ (*tos->ptr)(); } else { static uint8_t i; for(i = 0;i< tos->entry.length;i++){ (tos->entry.ptr[i])(); } } } else if ((tos->entry.flags & 0x40U) != 0){ if(STATE){ } }} const DictionaryNode interpret_node = {.next = ((void*)0), .entry = {.flags = 0|0x10U, .length = 1, .ptr = (void*)&interpret}}
# 212 "<stdin>"
HAL
